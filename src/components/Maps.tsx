...
export default function MapsComponent({ position, setPosition, setIsValidZone }) {
    ...
    function DraggableMarker({
        ...
        // Throttled function to validate zone
        const validateZone = throttle(async (lat: number, lng: number) => {
            try {
        ...
        const eventHandlers = {
            async dragend(e: any) {
                ...
                const isValid = await validateZone(newPos.lat, newPos.lng);
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden relative">
            <MapSearch searchText={searchText} setSearchText={setSearchText} setMarkerPosition={async (value) => {
                ...
                if (!zoneData?.checkZoneRestrictions?.selectedZone) {
                    toast.error("You are outside our service area. Redirecting to the default Berlin location.")
